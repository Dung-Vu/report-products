window.impactProjects = [
  {
    ...
    hook: "Hub vận hành trên Odoo quản trị 6,677 sản phẩm và 3,539 BOM không phụ thuộc thác tác rời rạc.",
    problem: "Odoo có 6,677 sản phẩm và 3,539 BOM nhưng thiếu lớp giám sát tập trung — kiểm tra, chỉnh sửa hàng loạt và cảnh báo vận hành phụ thuộc người nhớ làm. Không có lịch sử thay đổi tập trung và không có cách phát hiện sản phẩm thiếu thiết lập.",
    ...
    verifiedMetrics: [
      ...
      "98 thao tác dữ liệu đã ghi lại: 34 cập nhật cost, 32 xử lý BOM, 32 cập nhật pricelist — audit.db",
    ],
  },
  {
    id: "ord-price-lookup",
    ...
    hook: "Hệ thống tra cứu giá có audit ghi lại 478 lượt tra từ 2025-12 — Sales dùng thật, không phải builder test.",
    problem: "Sales cần phản hồi giá nhanh nhưng dữ liệu rải rác giữa Excel, admin và nhiều người nắm khác nhau. Không có cách tra theo role và không có lịch sử thay đổi giá.",
  ...
  },
  {
    ...
    hook: "Nền tảng tra cứu và kiểm kho tập trung phục vụ 4,811 phiếu xuất nhập kho trên Odoo.",
    problem: "Kho vận hành 4,811 phiếu xuất nhập từ 22/09/2025 nhưng thiếu lớp công cụ để tra cứu tồn kho và kiểm kho theo quy trình chuẩn. Khó phân quyền admin/counter và truy vết thao tác.",
    ...
    verifiedMetrics: [
      "4,811 done stock pickings trong Odoo từ 22/09/2025 — Odoo 2026-05-15",
  ...
  },
  {
    ...
    problem: "In label nhập kho và QC cần lấy dữ liệu từ Odoo — làm tay mất nhiều bước và thiếu lịch sử QC tập trung. Không có cách xuất báo cáo chuẩn từ các phiếu nhập.",
    ...
    verifiedMetrics: [
      "7+ QC batches lưu trong session — Docker logs 2026-05-15",
  ...
  },
  {
    ...
    problem: "Shopify, Odoo, Zalo ZNS, Telegram và các tác vụ định kỳ cần chạy ổn định — rải rác thành nhiều service riêng thì khó debug và khó thay đổi. Không có nơi xem tổng thể workflow nào đang chạy.",
  ...
  },
  {
    ...
    hook: "Worker tự động phân bổ OP và lưu 983 chứng từ bill/VAT hàng tháng vào kho.",
    problem: "Gán OP, xử lý bill và lưu PDF cuối tháng là các việc lặp lại, dễ sai tên file và tốn thời gian khi làm thủ công. Không có archive tập trung dễ truy cập và chia sẻ.",
  ...
  },
  {
    ...
    problem: "Chứng từ nằm trong thư mục local dùng được nhưng không có cách truy cập thống nhất và khó gắn vào pipeline tự động. Chia sẻ file phụ thuộc copy tay hoặc đường dẫn local.",
  ...
  },
  {
    ...
    problem: "Công thức tính kích thước rèm, lượng vải và chuyển Excel/PDF sản xuất có nhiều biến — làm tay dễ sai và khó chuẩn hóa output. Không có nơi kiểm tra công thức chung giữa các người làm.",
    built: "Flask web + FastAPI + pandas/openpyxl/PyMuPDF với form, upload Excel/PDF và xuất layout BON. 5 POST /process_production_pdf xác nhận đang được dùng production.",
    verifiedMetrics: [
  ...
  },
  {
    ...
    problem: "Tạo hình lifestyle bằng AI dễ bị lệch brand và khó lặp lại nếu prompt do mỗi người tự viết theo cách riêng. Không có quy trình chuẩn từ brief → generate → refine.",
    ...
    verifiedMetrics: ["ordinaire-brief-builder đang chạy port 9001, service healthy"],
  },
  {
    id: "tunnel-master",
    name: "Tunnel Master",
    ...
    problem: "Mỗi app cần truy cập riêng sẽ rối routing và tăng rủi ro nếu không có lớp quản lý tunnel chung. Cấu hình riêng từng service khó maintain và khó debug khi sự cố.",
  ...
  },
  {
    ...
    problem: "Cần expose action-product qua tunnel riêng mà không gắn thêm routing logic vào backend chính. Thay đổi đường truy cập không được ảnh hưởng backend.",
    ...
    verifiedMetrics: ["action-local-bridge up 7 days", "port 5504 → action-product:5004"],
  },
];
...
window.futureProjects = [
  {
    ...
    caution: "Chưa đưa vào thành tích hoàn tất. Chỉ dùng trong phần roadmap năng lực tiếp theo cho HR và quản trị tài khoản.",
  },
];
