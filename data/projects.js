...
const projectNarratives = {
  "bonario-product-hub": {
    ...
    operation: "Người dùng đăng nhập vào hub, chọn module nghiệp vụ như Checker, BOM, Pricelist, Description, Cambodia, Inventory, CRM hoặc Tracking. Frontend gọi backend Flask; backend xử lý qua service layer và đồng bộ dữ liệu với Odoo JSON-RPC.",
    features: [
    ...
    practicalUse: "Dùng như điểm thao tác chính cho vận hành sản phẩm, Supply Chain, sales/SC report owner và admin hệ thống.",
    businessValue: "Giảm thao tác thủ công trên Odoo, chuẩn hóa dữ liệu sản phẩm, gom nhiều luồng nghiệp vụ vào một nơi và giúp quản lý nhìn được tình trạng vận hành.",
    performance: "Tăng tốc các việc lặp lại như BOM, giá, mô tả và kiểm tra dữ liệu; đồng thời giảm rủi ro sai lệch nhờ audit log, workflow định kỳ và monitoring.",
  },
  "bonario-stock-management": {
    what: "Bonario Stock Management là hệ thống tra cứu tồn kho và kiểm kho hàng tháng. Repo này thay thế cách quản lý rời rạc bằng file hoặc thao tác thủ công.",
    operation: "Frontend cho người dùng chọn kho, tìm sản phẩm, xem tồn hệ thống từ Odoo qua backend proxy, tạo phiên kiểm kho theo tháng/kho và lưu số đếm thực tế vào PostgreSQL.",
    features: [
    ...
    practicalUse: "Phục vụ nhân viên kiểm kho, admin quản lý session và team vận hành kho khi cần kiểm tra on-hand stock hoặc xử lý stocktake hàng tháng.",
    businessValue: "Chuẩn hóa quy trình kiểm kho, giảm rủi ro thất lạc dữ liệu, tạo nguồn dữ liệu tập trung và hỗ trợ nhiều người cùng làm việc.",
    performance: "Rút ngắn thời gian tra cứu, giảm nhập liệu lặp lại, giảm sai lệch giữa số đếm thực tế và số hệ thống nhờ lưu lịch sử phiên kiểm kho.",
  },
  "ord-price-lookup": {
    what: "ORD Price Lookup System là hệ thống tra cứu và quản trị giá sản phẩm nội thất ORD cho sales, admin và Supply Chain.",
    operation: "Sales tra cứu giá qua frontend; admin/SC cập nhật cost, fabric, shipping và packaging. Backend Flask lưu dữ liệu vào PostgreSQL, quản lý import/export và lịch sử thay đổi.",
    features: [
    ...
    practicalUse: "Dùng trong luồng báo giá và kiểm tra giá nội bộ, giúp sales có nguồn tra cứu thống nhất thay vì hỏi thủ công hoặc xem nhiều file.",
    businessValue: "Giảm sai lệch giá bán, tăng tốc phản hồi cho sales và giúp công ty kiểm soát COGS/giá bán có hệ thống.",
    performance: "Tăng hiệu suất tra cứu và cập nhật giá; giảm thời gian tổng hợp Excel, giảm lỗi do dữ liệu giá phân tán.",
  },
  "in-label-pdf": {
    ...
    operation: "Frontend React cho người dùng tra cứu phiếu nhập, backend Express gọi Odoo JSON-RPC để lấy dữ liệu và tạo các file PDF/label theo nghiệp vụ.",
    features: [
    ...
    practicalUse: "Dùng tại khâu kho/QC khi cần in label nhanh, lấy thông tin phiếu nhập và xử lý tài liệu phục vụ kiểm hàng.",
    businessValue: "Giảm thời gian thao tác trên Odoo và file thủ công, giúp kho xử lý phiếu nhập nhất quán hơn.",
    performance: "Tăng tốc in label, giảm lỗi copy dữ liệu, hỗ trợ xử lý batch tốt hơn trong cao điểm nhập hàng.",
  },
  "auto-workflow": {
    ...
    operation: "Ứng dụng Flask nhận trigger hoặc chạy scheduler, sau đó điều phối logic giữa Shopify, Odoo, Zalo ZNS, Telegram và các tác vụ nội bộ.",
    features: [
    ...
    practicalUse: "Dùng cho các luồng automation vận hành cần chạy đều, gửi thông báo hoặc đồng bộ dữ liệu giữa nhiều hệ thống.",
    businessValue: "Giảm phụ thuộc vào thao tác thủ công, tránh workflow nằm rải rác và giúp logic nghiệp vụ được quản lý bằng code rõ ràng.",
    performance: "Tăng độ ổn định cho các tác vụ lặp lại, giảm thời gian theo dõi thủ công và giảm lỗi do quên chạy việc định kỳ.",
  },
  "op-round-robin": {
    ...
    operation: "Worker kết nối Odoo, đọc quotation hoặc dữ liệu chứng từ, áp dụng logic phân bổ/đổi tên/xử lý PDF rồi ghi kết quả phục vụ back office.",
    features: [
    ...
    practicalUse: "Dùng trong back office để giảm thao tác lặp lại khi phân bổ OP và chuẩn hóa bộ hồ sơ chứng từ.",
    businessValue: "Giúp quy trình xử lý quotation và chứng từ đều hơn, giảm phụ thuộc vào phân bổ thủ công.",
    performance: "Tiết kiệm thời gian cho các tác vụ lặp lại, giảm lỗi khi xử lý nhiều bill/chứng từ cùng lúc.",
  },
  "bills-server": {
    what: "Bills Server là lớp truy cập bill/chứng từ qua web, phục vụ tra cứu và chia sẻ file nội bộ từ thư mục lưu trữ.",
    operation: "Nginx serve thư mục chứng từ qua HTTP nội bộ, giúp các file bill/VAT/phiếu giao nhận mở nhanh bằng trình duyệt.",
    features: [
    ...
    practicalUse: "Dùng như lớp truy cập nhanh cho file chứng từ thay vì phải mở trực tiếp folder trên máy/server.",
    businessValue: "Tăng tính sẵn sàng của dữ liệu chứng từ, giúp back office chia sẻ và kiểm tra file nhanh hơn.",
    performance: "Giảm thời gian tìm file và giảm phụ thuộc vào đường dẫn local, đặc biệt khi nhiều người cùng cần tra cứu.",
  },
  "calculate-curtain-size": {
    what: "Curtain Size Calculator là công cụ tính kích thước rèm và lượng vải cần order cho sản xuất.",
    operation: "Người dùng nhập thông tin rèm hoặc upload file Excel/PDF. Hệ thống Flask/FastAPI xử lý công thức, chuyển đổi dữ liệu và xuất layout Excel BON cho sản xuất.",
    features: [
    ...
    practicalUse: "Dùng khi cần tính nhanh kích thước rèm, chuẩn bị file sản xuất hoặc xử lý danh sách đơn hàng rèm số lượng lớn.",
    businessValue: "Giảm lỗi tính thủ công, tăng tốc chuẩn bị đơn sản xuất và hỗ trợ team sản xuất có format dữ liệu thống nhất.",
    performance: "Rút ngắn thời gian xử lý file Excel/PDF và giảm sai số khi tính lượng vải cần order.",
  },
  "visual-brief-builder": {
    ...
    operation: "Người dùng chọn category, use case, material, mood, ánh sáng, tỷ lệ ảnh và ảnh tham chiếu. App dựng prompt theo rulebook, gửi sang API sinh ảnh và theo dõi kết quả.",
    features: [
    ...
    practicalUse: "Dùng bởi marketing/brand để tạo brief, sinh option, rà ảnh và chỉnh ảnh lifestyle sản phẩm.",
    businessValue: "Chuẩn hóa hình ảnh theo brand rules, giảm lệch tone hình ảnh và tăng tốc vòng lặp sản xuất nội dung.",
    performance: "Giảm thời gian viết prompt thủ công, tăng tốc brief -> tạo ảnh -> chỉnh ảnh và giúp scale nội dung visual đều hơn.",
  },
  "tunnel-master": {
    what: "Tunnel Master là module quản lý Cloudflare Tunnel tập trung cho nhiều app nội bộ Bonario.",
    operation: "Module quản lý cấu hình tunnel/routing để đưa các service Docker local ra domain có kiểm soát, không cần mở port router trực tiếp.",
    features: [
    ...
    practicalUse: "Dùng khi cần đưa các app Docker local lên domain để người liên quan truy cập tạm thời từ bên ngoài mạng local.",
    businessValue: "Tăng tốc chia sẻ app nội bộ, giảm rủi ro cấu hình mạng thủ công và tạo nền tảng publish service nhất quán.",
    performance: "Rút ngắn thời gian mở truy cập app, giảm lỗi cấu hình từng app riêng lẻ và giúp kiểm soát routing tập trung.",
  },
  "action-local-bridge": {
    ...
    operation: "Container listen port 5504 và forward TCP traffic vào action-product:5004 trong Docker network, tách phần routing khỏi app chính.",
    features: [
    ...
    practicalUse: "Dùng để mở truy cập Product Hub qua một đường bridge/tunnel riêng mà không phải chỉnh code backend.",
    businessValue: "Giúp kiểm thử hoặc mở truy cập service linh hoạt hơn, giảm ảnh hưởng lên app chính khi thay đổi routing.",
    performance: "Thiết lập nhanh, ít tài nguyên, phù hợp vai trò utility infrastructure cho môi trường nội bộ.",
  },
};
