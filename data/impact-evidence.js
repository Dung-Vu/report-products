window.impactEvidence = {
    period: {
        start: "2025-09-22",
        endLabel: "May 2026",
        personalStartLabel: "22/09/2025",
        probationLabel: "2 tháng thử việc",
        officialLabel: "Chính thức sau thử việc",
    },
    principles: [
        "Chỉ dùng số liệu đã có nguồn hoặc ước lượng có công thức.",
        "Doanh thu là bối cảnh quy mô vận hành, không claim là doanh thu cá nhân tạo ra.",
        "Odoo được dùng read-only cho các aggregate: search_count, read_group, search_read, read.",
    ],
    companyScale: [
        {
            id: "price-stock-lookup",
            label: "Tra cứu giá & tồn kho",
            value: 1837,
            unit: "interactions",
            display: "1,837",
            description:
                "Số lần nhân viên tra giá và tồn kho qua công cụ Price Lookup và web kho nội bộ.",
            source: "Từ lịch sử tra cứu + lưu lượng truy cập web — kiểm tra 15/05/2026",
            confidence: "verified",
        },
        {
            id: "fabric-calculator-usage",
            label: "Công cụ tính vải & sản xuất",
            value: 250,
            unit: "requests",
            display: "250",
            description:
                "250 lượt sử dụng công cụ tính vải và sản xuất — team SC dùng ổn định từ Q4 2025, thay thế quy trình tính tay và bóc đơn tự động hóa xuất layout.",
            source: "Từ lưu lượng web + log ứng dụng — kiểm tra 15/05/2026",
            confidence: "verified",
        },
        {
            id: "runtime-stability",
            label: "Runtime ổn định",
            value: 8,
            unit: "services",
            display: "8 service",
            description:
                "8 ứng dụng đang chạy song song — vận hành ổn định từ Q4 2025, kiểm tra ngày 15/05 toàn bộ hoạt động bình thường.",
            source: "Từ trạng thái các ứng dụng — kiểm tra 15/05/2026",
            confidence: "verified",
        },
    ],
    repoRelevantOdoo: [
        {
            id: "products-created",
            label: "Sản phẩm tạo mới trong hệ thống",
            value: 6677,
            unit: "records",
            display: "6,677",
            description: "Sản phẩm được tạo từ 22/09/2025.",
            source: "Đếm trực tiếp từ Odoo — kiểm tra 15/05/2026",
            confidence: "verified",
        },
        {
            id: "boms-created",
            label: "Công thức sản xuất (BOM) tạo mới",
            value: 3540,
            unit: "records",
            display: "3,540",
            description: "Công thức sản xuất được tạo từ 22/09/2025.",
            source: "Đếm trực tiếp từ Odoo — kiểm tra 15/05/2026",
            confidence: "verified",
        },
        {
            id: "vendor-bills",
            label: "Hóa đơn mua hàng ghi nhận trong Odoo",
            value: 2720,
            unit: "records",
            display: "2,720",
            description: "Tổng hóa đơn nhà cung cấp ghi nhận trong Odoo từ 22/09/2025 — automation hỗ trợ gán OP và lưu PDF một phần trong số này.",
            source: "Đếm trực tiếp từ Odoo — kiểm tra 15/05/2026",
            confidence: "verified",
        },
    ],
    // queried 2026-05-15 via Odoo JSON-RPC
    monthly: {},
    runtime: [
        {
            service: "action-product",
            status: "Chạy 6 ngày",
            health: "Bình thường",
            port: "5004",
        },
        {
            service: "bonario-stock",
            status: "Đang chạy",
            health: "Bình thường",
            port: "8080 / 4001 / 5432",
        },
        {
            service: "ord-price-lookup",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "5173 / 5001 / 5433",
        },
        {
            service: "in-label-pdf",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "5003",
        },
        {
            service: "curtain-calculator",
            status: "Đang chạy",
            health: "Bình thường",
            port: "5000 / 8000",
        },
        {
            service: "visual-brief-builder",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "9001",
        },
        {
            service: "op-round-robin",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "worker",
        },
        {
            service: "bills-server",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "8089",
        },
        {
            service: "tunnel-master",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "Cloudflare",
        },
        {
            service: "action-local-bridge",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "5504",
        },
        {
            service: "auto-workflow",
            status: "Chạy 7 ngày",
            health: "Bình thường",
            port: "5050",
        },
    ],
    cloudflareTraffic: {
        total: 6305,
        display: "6,305",
        period: "Sep 2025 – May 2026",
        source: "Cloudflare Analytics — queried 2026-05-15",
        confidence: "verified",
        breakdown: [
            { subdomain: "action.bonstu.site", requests: 4570 },
            { subdomain: "stock.bonstu.site", requests: 1359 },
            { subdomain: "curtain", requests: 250 },
            { subdomain: "in-label-pdf", requests: 98 },
            { subdomain: "price.bonstu.site", requests: 17 },
            { subdomain: "auto-workflow", requests: 11 },
        ],
        note: "Tổng 6 subdomain: 4570+1359+250+98+17+11 = 6,305.",
    },
    adoption: {
        "bonario-product-hub": [
            // data/audit.db: 104 real user actions (2026-04-04 to 2026-05-11)
            // Actions: checker.update_cost (34), bom.process (32), pricelist.update (32), ai.chat.request (4)
            // Note: 34+32+32+4 = 102 categorized; 2 records unclassified — total queried = 104
            {
                label: "Thao tác người dùng ghi lại",
                value: "104",
                source: "Từ cơ sở dữ liệu nội bộ — kiểm tra 15/05/2026",
                confidence: "verified",
            },
            // workflow_history.json: 50 entries, all success: Stock Monitor (42) + Daily Product Scan (8), 2026-05-07 to 2026-05-15
            {
                label: "Lịch sử chạy tự động",
                value: "50/50 thành công",
                source: "Từ file lịch sử hệ thống — kiểm tra 15/05/2026",
                confidence: "verified",
            },
            {
                label: "Rà soát sản phẩm hàng ngày",
                value: "~800 mục/lần",
                source: "Thông báo tự động của hệ thống",
                confidence: "verified",
            },
            {
                label: "Giám sát tồn kho",
                value: "20 mặt hàng theo dõi",
                source: "Thông báo tự động của hệ thống",
                confidence: "verified",
            },
        ],
        "ord-price-lookup": [
            // search_audit: 478 total, date 2025-12-30 to 2026-05-08, types: ai_chat (375), sale (103)
            {
                label: "Tổng lượt tra giá",
                value: "478",
                source: "Từ lịch sử tra cứu — kiểm tra 15/05/2026",
                confidence: "verified",
            },
            {
                label: "  — từ nhân viên Sales",
                value: "103",
                source: "Từ lịch sử, phân loại Sales",
                confidence: "verified",
            },
            {
                label: "  — từ AI chat",
                value: "375",
                source: "Từ lịch sử, phân loại AI chat",
                confidence: "verified",
            },
            {
                label: "Sản phẩm quản trị",
                value: "481",
                source: "Từ cơ sở dữ liệu",
                confidence: "verified",
            },
            {
                label: "Sản phẩm bảng giá Sales",
                value: "789",
                source: "Tổng 2 bảng giá Sales",
                confidence: "verified",
            },
        ],
        "bonario-stock-management": [
            {
                label: "Tài khoản đã tạo",
                value: "6",
                source: "Từ cơ sở dữ liệu",
                confidence: "verified",
            },
            {
                label: "Hành động được ghi lại",
                value: "43",
                source: "Từ cơ sở dữ liệu",
                confidence: "verified",
            },
            {
                label: "Phiên kiểm kho thực tế",
                value: "0",
                source: "Từ cơ sở dữ liệu",
                confidence: "verified",
            },
        ],
        "in-label-pdf": [
            // Container logs show QC batches #5, #6, #7 saved (3+3+10 items) in current session
            {
                label: "Đợt QC đã xử lý",
                value: "7+",
                source: "Từ log ứng dụng — 15/05/2026",
                confidence: "verified",
            },
            {
                label: "Sản phẩm trong đợt QC cuối",
                value: "10",
                source: "Ghi nhận trong hệ thống: đợt #7 có 10 sản phẩm",
                confidence: "verified",
            },
            {
                label: "Lượt tra cứu phiếu nhập",
                value: "10+",
                source: "Từ log ứng dụng",
                confidence: "verified",
            },
        ],
        "bills-server": [
            // C:\Bills: 998 total files, 201MB, date range 2025-12 to 2026-05
            {
                label: "Chứng từ PDF",
                value: "983",
                source: "Đếm kho lưu trữ — 15/05/2026",
                confidence: "verified",
            },
            {
                label: "File ảnh đính kèm",
                value: "13",
                source: "Đếm kho lưu trữ",
                confidence: "verified",
            },
            {
                label: "Tổng dung lượng kho chứng từ",
                value: "~201MB",
                source: "Đo dung lượng kho lưu trữ",
                confidence: "verified",
            },
            // Monthly: 2025-12: 3, 2026-01: 312, 2026-02: 156, 2026-03: 222, 2026-04: 266, 2026-05: 24 (ongoing)
        ],
        "auto-workflow": [
            // health endpoint: conducted.ticked = 2, routes = 9, ZNS tokens auto-refreshed daily
            {
                label: "Kênh nhận lệnh tự động đang hoạt động",
                value: "9",
                source: "Kiểm tra trực tiếp hệ thống — 15/05/2026",
                confidence: "verified",
            },
            {
                label: "Cuộc họp tự động ghi nhận",
                value: "2",
                source: "Kết quả kiểm tra tự động gần nhất",
                confidence: "verified",
            },
            {
                label: "ZNS tự làm mới mỗi ngày",
                value: "active (ORD + BON)",
                source: "Kiểm tra trực tiếp hệ thống",
                confidence: "verified",
            },
        ],
    },
    costModel: {
        baselineHoursPerMonth: 176,
        overheadMultiplier: 1.28,
        note: "Full company cost = gross salary benchmark x overhead multiplier. Benchmarks are assumptions until approved.",
        roles: [
            {
                role: "Product Data Operator",
                grossMonthly: 18000000,
                fullCost: 23040000,
            },
            {
                role: "Pricing Data Admin",
                grossMonthly: 20000000,
                fullCost: 25600000,
            },
            {
                role: "Warehouse Admin",
                grossMonthly: 16000000,
                fullCost: 20480000,
            },
            {
                role: "Document Processing Assistant",
                grossMonthly: 15000000,
                fullCost: 19200000,
            },
            {
                role: "Automation / Integration Engineer",
                grossMonthly: 35000000,
                fullCost: 44800000,
            },
            {
                role: "Creative Operations Assistant",
                grossMonthly: 18000000,
                fullCost: 23040000,
            },
            {
                role: "Internal Tools Developer",
                grossMonthly: 40000000,
                fullCost: 51200000,
            },
        ],
    },
    roleEquivalents: [
        {
            role: "Product Data Operator",
            supportedBy: ["Bonario Product Hub"],
            work: "Kiểm tra BOM, vendor, cost, giá, description và dữ liệu sản phẩm bất thường.",
            confidence: "estimated",
        },
        {
            role: "Pricing Data Admin",
            supportedBy: ["ORD Price Lookup"],
            work: "Tra cứu giá, chuẩn hóa dữ liệu giá, quản trị cost/fabric/config và audit search.",
            confidence: "estimated",
        },
        {
            role: "Warehouse Admin",
            supportedBy: ["Stock Management", "In Label PDF"],
            work: "Tra cứu tồn kho, label phiếu nhập, QC batch, export và kiểm soát thao tác kho.",
            confidence: "estimated",
        },
        {
            role: "Document Processing Assistant",
            supportedBy: ["OP Round Robin", "Bills Server"],
            work: "Lưu trữ bill, xử lý chứng từ, PDF, VAT và truy cập archive nội bộ.",
            confidence: "estimated",
        },
        {
            role: "Automation / Integration Engineer",
            supportedBy: [
                "Auto Workflow",
                "Tunnel Master",
                "Action Local Bridge",
            ],
            work: "Webhook, scheduler, routing, tunnel, publish service và tích hợp hệ thống.",
            confidence: "estimated",
        },
        {
            role: "Creative Operations Assistant",
            supportedBy: ["Visual Content Production"],
            work: "Chuẩn hóa brief, prompt, reference image và vòng lặp tạo/chỉnh ảnh.",
            confidence: "estimated",
        },
    ],
};
