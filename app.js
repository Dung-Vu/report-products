(function () {
  ...
  function animateCounter(el, target, suffix, duration) {
    ...
    function step(now) {
      ...
      el.textContent = new Intl.NumberFormat("vi-VN").format(current) + (suffix || "");
  ...
  const SYSTEM_GROUPS = [
    { id: "product-data", label: "Dữ liệu sản phẩm", eyebrow: "Product Ops",
      summary: "Quản trị product, BOM, cost, giá và daily scan qua hub tập trung.",
      projectIds: ["bonario-product-hub", "ord-price-lookup"], accent: "#0f766e" },
    { id: "warehouse-qc", label: "Kho / QC", eyebrow: "Warehouse",
      summary: "Tra tồn kho, in label, QC batch nhập kho và nền tảng kiểm kho.",
      projectIds: ["bonario-stock-management", "in-label-pdf"], accent: "#0369a1" },
    { id: "bills-docs", label: "Chứng từ", eyebrow: "Back Office",
      summary: "Bill archive 983 PDF, round-robin OP, VAT và web file server nội bộ.",
      projectIds: ["op-round-robin", "bills-server"], accent: "#7c3aed" },
    { id: "automation", label: "Automation", eyebrow: "Integration",
      summary: "Webhook, scheduler daily, Zalo ZNS auto-refresh, Telegram RFID, 9 routes live.",
      projectIds: ["auto-workflow"], accent: "#c2410c" },
    { id: "infrastructure", label: "Hạ tầng", eyebrow: "Platform",
      summary: "Cloudflare tunnel, Docker network, TCP bridge — publish app nội bộ không cần mở port.",
      projectIds: ["tunnel-master", "action-local-bridge"], accent: "#334155" },
    { id: "production-tools", label: "Sản xuất", eyebrow: "Production",
      summary: "Tính kích thước rèm, xử lý Excel/PDF sản xuất và export layout BON.",
      projectIds: ["calculate-curtain-size"], accent: "#0891b2" },
    { id: "marketing-ai", label: "Creative AI", eyebrow: "Marketing",
      summary: "Brief builder, model selection, ratio logic, reference và refinement preset cho hình lifestyle.",
      projectIds: ["visual-brief-builder"], accent: "#be123c" },
  ];

  const COUNTERS = [
    { value: 983, suffix: "", label: "Chứng từ PDF tự động hóa", sub: "op-round-robin xử lý · bills-server serve · 201MB", large: true },
    { value: 6305, suffix: "", label: "Lượt truy cập domain", sub: "action 4,570 · stock 1,359 · curtain 250 · label 98 · 6 subdomain Bonario · Cloudflare Sep~May" },
    { value: 50, suffix: "/50", label: "Workflow tự động (0 thất bại)", sub: "42 Stock Monitor + 8 Daily Scan · workflow_history.json · 2026-05-07~05-15" },
    { value: 9, suffix: "", label: "Webhook routes đang live", sub: "Shopify · Odoo · Zalo · Telegram · auto-workflow · action.bonstu.site: 4,570 req" },
  ];

  const TECH_STACK = [
    { cat: "Backend", items: ["Python · Flask", "Node.js · Express", "Odoo XML-RPC"] },
    { cat: "Frontend", items: ["React 18", "Vanilla JS", "Recharts"] },
    { cat: "Database", items: ["PostgreSQL", "SQLite", "Odoo ORM"] },
    { cat: "Infra", items: ["Docker · Compose", "Nginx", "Cloudflare Tunnel"] },
    { cat: "Tích hợp", items: ["Shopify Webhook", "Zalo ZNS API", "Telegram Bot", "PDF · OCR"] },
  ];
  ...
  const ROADMAP = [
    {
      ...
      items: ["Prometheus + Grafana cho toàn Docker stack", "Telegram alert tự động khi service down", "Log aggregation tập trung"],
    },
    {
      phase: "Trung hạn",
      accent: "#2563eb",
      ...
      items: ["AI tự classify & fix ~800 lỗi sản phẩm/ngày từ Daily Scan", "AI draft báo cáo tồn kho cuối tháng tự động", "NLP query kho: ‘còn bao nhiêu SKU X?’"],
    },
    {
      phase: "Team scale",
      accent: "#7c3aed",
      ...
      items: ["API docs + runbook cho từng service", "Incident playbook chuẩn hóa", "Onboarding guide cho developer mới"],
    },
  ];

  const STORY_ARC = [
    { year: "2019–05/2024", label: "Kỹ sư CNTT, GPA 2.03", note: "FE căn bản. IT thực tế chưa bao giờ chạm." },
    { year: "06/2024–08/2025", label: "Gap · chuẩn bị du học", note: "14 tháng. Không viết một dòng code production." },
    { year: "22/09/2025", label: "Vào Bonario · thử việc", note: "Học Odoo, đọc codebase, hiểu quy trình kho." },
    { year: "Tháng 10/2025", label: "Deploy đầu tiên", note: "ORD Price Lookup live — React + Flask + PostgreSQL." },
    { year: "Tháng 11–12/2025", label: "Automation stack", note: "9 webhook routes, scheduler daily, Telegram bot." },
    { year: "Tháng 1–3/2026", label: "Infrastructure layer", note: "Docker Compose, Cloudflare Tunnel, bills archive." },
    { year: "Tháng 5/2026", label: "11 hệ thống production", note: "Odoo · Docker · PostgreSQL · Verified.", highlight: true },
  ];
    ...
    return `
      <section class="scene scene-hook" id="scene-0" aria-label="Mở đầu">
        <div class="hook-inner">
          <div class="hook-text">
            <p class="hook-line1">Hơn một năm không chạm code.</p>
            <p class="hook-line2">Tám tháng tại Bonario — 11 hệ thống đang chạy.</p>
          </div>
          <p class="hook-sub">Từ 22/09/2025 &nbsp;·&nbsp; Bonario</p>
          <div class="hook-stats">
            ${COUNTERS.map((c) => `
            <div class="hook-stat">
              <strong class="hook-stat-num">${new Intl.NumberFormat("vi-VN").format(c.value)}${c.suffix || ""}</strong>
        ...
        <div class="scale-inner">
          ...
          <div class="value-callout">
            ...
            <div class="value-callout-items">
              ...
              <span><strong>~2.7h</strong> kiểm tra thủ công/ngày → Daily Scan phát hiện ~800 lỗi</span>
              <span><strong>100%</strong> scheduler không thất bại — 50/50 lần chạy</span>
              <span><strong>6.305</strong> request được phục vụ qua 6 subdomain live</span>
            </div>
          </div>
        </div>
      </section>`;
  ...
  function buildCloseScene() {
    const arcItems = STORY_ARC.map((item) => `
      <div class="arc-item${item.highlight ? " arc-highlight" : ""}">
        ...
        <div class="close-inner">
          ...
          <div class="close-statement">
            ...
            <p class="close-body">
              ...
              ${projects.length} hệ thống đang chạy từ 22/09/2025.
            ...
            <div class="close-meta">
              <span>22/09/2025 — ${monthYear}</span>
  ...
  function renderPanelContent(groupId) {
    ...
    if (!group) return "";
    const groupProjects = group.projectIds.map((id) => projects.find((p) => p.id === id)).filter(Boolean);
    const projectBlocks = groupProjects.map((proj) => {
      const metrics = (proj.verifiedMetrics || []).map((m) => `<li>${escapeHtml(m)}</li>`).join("");
      return `
  ...
  function startCounters() {
    ...
    document.querySelectorAll("[data-target]").forEach((el) => {
      animateCounter(el, parseInt(el.dataset.target, 10), el.dataset.suffix || "", 1800);
    });
  }
  ...
  function closePanel() {
    ...
    document.querySelectorAll(".node-btn").forEach((btn) => btn.classList.remove("node-active"));
  }
  ...
  function setupObservers() {
    ...
    if (hook && dotNav) {
      ...
      }, { threshold: 0.4 }).observe(hook);
    }
    const scale = document.getElementById("scene-1");
    if (scale) {
      new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting) startCounters();
      }, { threshold: 0.3 }).observe(scale);
    }
    document.querySelectorAll(".scene").forEach((scene) => {
      ...
      }, { threshold: 0.5 }).observe(scene);
    });
    // Entrance animations: add scene-in when scene enters viewport
    document.querySelectorAll(".scene").forEach((scene) => {
      new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting) scene.classList.add("scene-in");
  ...
  function bindEvents() {
    document.querySelectorAll("[data-scroll-to]").forEach((btn) => {
      btn.addEventListener("click", () => scrollToScene(parseInt(btn.dataset.scrollTo, 10)));
    });
    document.querySelectorAll(".dot[data-scene]").forEach((dot) => {
      dot.addEventListener("click", () => scrollToScene(parseInt(dot.dataset.scene, 10)));
    });
    document.querySelectorAll(".node-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        const panel = document.getElementById("side-panel");
        if (panel?.classList.contains("panel-open") && btn.classList.contains("node-active")) {
          closePanel();
        } else {
  ...
  function init() {
    ...
    requestAnimationFrame(() => document.getElementById("scene-0")?.classList.add("scene-in"));
  }
